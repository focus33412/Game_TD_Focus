...
namespace Kinnly
{
    public class PlayerTools : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            playerMovement = Player.Instance;
            playerVisual = Player.Instance.gameObject.GetComponent<PlayerVisual>();

            // Пытаемся получить аниматор из PlayerVisual
            if (playerVisual != null)
            {
                playerAnimator = playerVisual.animator;
            }
            
            // Если аниматор все еще null, пытаемся получить его напрямую
            if (playerAnimator == null)
            {
                playerAnimator = player.GetComponent<Animator>();
            }

            // Если аниматор все еще null, выводим предупреждение
            if (playerAnimator == null)
            {
                Debug.LogWarning("Animator не найден! Убедитесь, что компонент Animator прикреплен к объекту или доступен через PlayerVisual.");
            }
        }
        ...
        private void UseTools()
        {
            ...
            if (currentlySelectedItem.isTools && playerAnimator != null)
            {
